...
uint8_t CalCrc8(uint8_t crc, uint8_t *msg, unsigned long msgLen) {
    uint8_t Byte = 0;
...
uint16_t CalCrc16(uint16_t crc, uint8_t *msg, unsigned long msgLen) {
    uint8_t chData;
    ...
    while (msgLen--) {
        chData = *msg++;
        (crc) = ((uint16_t)(crc) >> 8) ^ CRC16Table[((uint16_t)(crc) ^ (uint16_t)(chData)) & 0x00ff];
    }
