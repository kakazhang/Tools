...
Dwfl_Memory_Callback *g_memory_callback = NULL;
void *g_memory_callback_arg = NULL;
Dwfl* g_dwfl = NULL;

inline bool read_addrs (void *buffer, GElf_Addr vaddr, size_t n,
                        uint_fast8_t elfclass, uint_fast8_t elfdata,
                        GElf_Addr *read_vaddr, size_t *buffer_available, GElf_Addr *addrs)
{
  ...
  || vaddr - *read_vaddr + nb > *buffer_available)
	{
	  if (buffer != NULL)
		  (void) (*g_memory_callback) (g_dwfl, -1, &buffer, buffer_available, 0, 0,
			  g_memory_callback_arg);


  *read_vaddr = vaddr;
  int segndx = INTUSE(dwfl_addrsegment) (g_dwfl, vaddr, NULL);
  if (unlikely (segndx < 0)
	  || unlikely (! (*g_memory_callback) (g_dwfl, segndx,
						 &buffer, buffer_available,
						 vaddr, nb, g_memory_callback_arg)))
	return true;
	}
...
{
  g_memory_callback= memory_callback;
  g_memory_callback_arg = memory_callback_arg;
  g_dwfl = dwfl;
...
#endif
  if (unlikely (read_addrs (buffer, read_vaddr, 1, elfclass, elfdata, &read_vaddr,
  	            &buffer_available, addrs))) {
      if (buffer != NULL)
	      (void) (*memory_callback) (dwfl, -1, &buffer, &buffer_available, 0, 0,
			   memory_callback_arg);
	  return -1;
  ...
  while (next != 0 && ++iterations < dwfl->lookup_elts)
    {
      ...
      g_dwfl = dwfl;
	  if (read_addrs (buffer, next, 4, elfclass, elfdata, &read_vaddr,
  	            &buffer_available, addrs)) {
         if (buffer != NULL)
             (void) (*memory_callback) (dwfl, -1, &buffer, &buffer_available, 0, 0,
				 memory_callback_arg);
    ...
    if (buffer != NULL)
	    (void) (*memory_callback) (dwfl, -1, &buffer, &buffer_available, 0, 0, memory_callback_arg);
  return result;
}

static GElf_Addr
...
bool consider_phdr (GElf_Word type,
		  ...
		  GElf_Xword *dyn_filesz)
{
    ...
    if (dyn_bias == (GElf_Addr) -1
        ...
        && ((vaddr & (g_dwfl->segment_align - 1))
	        == (phdr & (g_dwfl->segment_align - 1))))
     {
         dyn_bias = phdr - vaddr;
         return *dyn_vaddr != 0;
     }
