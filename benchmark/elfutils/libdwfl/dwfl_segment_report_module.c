	     ...
	     const void *build_id, size_t build_id_len)
{
 ...
 inline bool segment_read (Dwfl *dwfl,int segndx,
 ...
 inline void release_buffer (Dwfl *dwfl, void **buffer, size_t *buffer_available,
				   Dwfl_Memory_Callback *memory_callback,
				   void *memory_callback_arg)
 {
   if (*buffer != NULL)
	 (void) segment_read (dwfl, -1, buffer, buffer_available, 0, 0, memory_callback, memory_callback_arg);
 }

void* g_buffer = NULL;
inline bool read_portion (Dwfl *dwfl,void **data, size_t *data_size,
			 GElf_Addr vaddr, size_t filesz,size_t buffer_available,GElf_Addr start,
			 size_t segment,
			 Dwfl_Memory_Callback *memory_callback,
			 void *memory_callback_arg)
{
 ...
 || (filesz == 0 && memchr (vaddr - start + g_buffer, '\0',
				buffer_available - (vaddr - start)) == NULL))
   {
 *data = NULL;
 *data_size = filesz;
 return segment_read (dwfl, addr_segndx (dwfl, segment, vaddr, false),
			  data, data_size, vaddr, filesz, memory_callback, memory_callback_arg);
   }

 /* We already have this whole note segment from our initial read.	*/
 *data = vaddr - start + g_buffer;
 *data_size = 0;
 return false;
}

inline void finish_portion (Dwfl *dwfl, void **data, size_t *data_size,
                                  Dwfl_Memory_Callback *memory_callback,
                                  void* memory_callback_arg)
{
 if (*data_size != 0)
   release_buffer (dwfl, data, data_size, memory_callback, memory_callback_arg);
}

inline void final_read (Dwfl *dwfl, size_t offset, GElf_Addr vaddr,
                                      size_t size,size_t segment,
                                      void *contents,
									  Dwfl_Memory_Callback *memory_callback,
									  void* memory_callback_arg)
{
    ...
    (void) segment_read (dwfl, addr_segndx (dwfl, segment, vaddr, false),
		   &into, &read_size, vaddr, size, memory_callback, memory_callback_arg);
}
...
Elf *g_elf = NULL;
int g_ndx = -1;
inline int finish(Dwfl *dwfl, void **buffer, size_t *buffer_available,
				   Dwfl_Memory_Callback *memory_callback,
				   void *memory_callback_arg)
{
	if (g_phdrsp)
        free (g_phdrsp);
    release_buffer (dwfl, buffer, buffer_available, memory_callback, memory_callback_arg);
    if (g_elf != NULL)
      elf_end (g_elf);
    if (g_fd != -1)
      close (g_fd);
...
extern bool consider_phdr (GElf_Word type,
		  GElf_Addr vaddr, GElf_Xword filesz, GElf_Addr phdr,
		  GElf_Addr dyn_bias, GElf_Addr* dyn_vaddr,
		  GElf_Xword *dyn_filesz);
#if 1
#define UNUSED __attribute__((unused))
int
dwfl_segment_report_module (Dwfl *dwfl UNUSED, int ndx UNUSED,
               const char *name UNUSED,
			    Dwfl_Memory_Callback *memory_callback UNUSED,
			    void *memory_callback_arg UNUSED,
			    Dwfl_Module_Callback *read_eagerly UNUSED,
			    void *read_eagerly_arg UNUSED,
			    const void *note_file UNUSED, size_t note_file_size UNUSED,
			    const struct r_debug_info *r_debug_info UNUSED)
{
    return 0;
}
#else
			    ...
			    const struct r_debug_info *r_debug_info)
{
...
#endif
  if (segment_read (dwfl, ndx, &buffer, &buffer_available,
		    start, sizeof (Elf64_Ehdr),memory_callback, memory_callback_arg)
      || memcmp (buffer, ELFMAG, SELFMAG) != 0) {
      g_phdrsp = phdrsp;
	  g_fd = fd;
	  g_elf = elf;
	  g_ndx = ndx;
	  return finish(dwfl, &buffer, &buffer_available,
  ...
  g_buffer = buffer;
  if (read_portion (dwfl, &ph_buffer, &ph_buffer_size,
  ...
  inline void consider_notes (GElf_Addr vaddr, GElf_Xword filesz)
  {
    ...
    g_buffer = buffer;
    if (read_portion (dwfl, &data, &data_size, vaddr, filesz,
		buffer_available, start, segment, memory_callback, memory_callback_arg))
      return;
    ...
    if (notes != data)
      free (notes);
    finish_portion (dwfl, &data, &data_size, memory_callback, memory_callback_arg);
  }
  ...
  finish_portion (dwfl, &ph_buffer, &ph_buffer_size, memory_callback, memory_callback_arg);
  ...
  if (dyn_filesz != 0 && dyn_filesz % dyn_entsize == 0
      && ! read_portion (dwfl, &dyn_data, &dyn_data_size, dyn_vaddr, dyn_filesz,
            buffer_available, start, segment, memory_callback, memory_callback_arg))
    {
  ...
  finish_portion (dwfl, &dyn_data, &dyn_data_size, memory_callback, memory_callback_arg);
  ...
  if (! name_is_final && dynstrsz != 0 && dynstr_vaddr != 0)
	...
	dynstrsz = 0;
	  ...
	  && ! read_portion (dwfl, &soname, &soname_size,
			     dynstr_vaddr + soname_stroff, 0,
			     buffer_available, start, segment,
			     memory_callback, memory_callback_arg))
	name = soname;
    }
  ...
  free (build_id);
  finish_portion (dwfl, &soname, &soname_size, memory_callback, memory_callback_arg);
  ...
  if (elf == NULL
      ...
      else
	{
	  ...
	  if (have < file_trimmed_end)
	    final_read (dwfl, have, start + have, file_trimmed_end - have,
	    segment, contents, memory_callback, memory_callback_arg);
	}
