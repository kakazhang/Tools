...
/* Reconstruct an ELF file by reading the segments out of remote memory
   ...
   segments.  */
GElf_Xword g_pagesize = 0;
size_t contents_size = 0;
GElf_Off segments_end = 0;
GElf_Off segments_end_mem = 0;
GElf_Addr loadbase = 0;
bool found_base = false;
GElf_Addr g_ehdr_vma = 0;
void *g_arg = NULL;
unsigned char* g_buffer = NULL;
typedef ssize_t (*read_memory_func) (void *arg, void *data,
						GElf_Addr address,
						size_t minread,
						size_t maxread);
read_memory_func g_func;

inline bool handle_segment (GElf_Addr vaddr, GElf_Off offset,
		  GElf_Xword filesz, GElf_Xword memsz)
{
    /* Sanity check the segment load aligns with the pagesize.  */
    if (((vaddr - offset) & (g_pagesize - 1)) != 0)
        return true;

    GElf_Off segment_end = ((offset + filesz + g_pagesize - 1)
		  & -g_pagesize);

    if (segment_end > (GElf_Off) contents_size)
        contents_size = segment_end;

    if (!found_base && (offset & -g_pagesize) == 0)
    {
        loadbase = g_ehdr_vma - (vaddr & -g_pagesize);
        found_base = true;
    }
...
inline bool handle_segment1 (GElf_Addr vaddr, GElf_Off offset,
			GElf_Xword filesz)
{
    GElf_Off start = offset & -g_pagesize;
    GElf_Off end = (offset + filesz + g_pagesize - 1) & -g_pagesize;
    if (end > (GElf_Off) contents_size)
        end = contents_size;
    int nread = (*g_func) (g_arg, g_buffer + start,
			(loadbase + vaddr) & -g_pagesize,
			end - start, end - start);
    return nread <= 0;
}
			...
			void *arg)
{
  ...
  switch (ehdr.e32.e_ident[EI_CLASS])
    ...
    case ELFCLASS32:
      for (uint_fast16_t i = 0; i < phnum; ++i)
	if ((*p32)[i].p_type == PT_LOAD)
	  if (handle_segment1 ((*p32)[i].p_vaddr, (*p32)[i].p_offset,
			      (*p32)[i].p_filesz))
	    goto read_error;
    ...
    case ELFCLASS64:
      for (uint_fast16_t i = 0; i < phnum; ++i)
	if ((*p64)[i].p_type == PT_LOAD)
	  if (handle_segment1 ((*p64)[i].p_vaddr, (*p64)[i].p_offset,
			      (*p64)[i].p_filesz))
	    goto read_error;
